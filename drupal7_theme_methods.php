...
$art_style = '';
$art_head = '';
function TheCPGroup_Final_SS_process_html(&$variables) {
    global $art_style, $art_head;
	$view = TheCPGroup_Final_SS_art_get_drupal_view();
	...
	$themePath = TheCPGroup_Final_SS_art_get_full_path_to_theme();
...
function TheCPGroup_Final_SS_breadcrumb($variables) {
 ...
 * @ingroup themeable
 */
function TheCPGroup_Final_SS_button($variables) {
 ...
 */
function TheCPGroup_Final_SS_preprocess_page(&$vars) {
  global $art_style, $art_head;
 ...
 * @ingroup themeable
 */
function TheCPGroup_Final_SS_menu_local_task($variables) {
 ...
 */
function TheCPGroup_Final_SS_feed_icon($variables) {
 ...
 * @ingroup themeable
 */
function TheCPGroup_Final_SS_node_preview($variables) {
  $node = $variables['node'];
 ...
 */
function TheCPGroup_Final_SS_art_links_woker_D7($content) {
  $result = '';
  ...
  foreach (array_keys($content['links']) as $name) {
	...
	if (isset($content['links'][$name]['#links'])) {
	  ...
	  if (is_array($links)) {
		$output = TheCPGroup_Final_SS_art_links_html_output_D7($links);
		if (!empty($output)) {
...
function TheCPGroup_Final_SS_art_terms_D7($content) {
	$result = NULL;
...
function TheCPGroup_Final_SS_art_links_html_output_D7($links) {
	$output = '';
...
*/
function TheCPGroup_Final_SS_form_alter(&$form, &$form_state, $form_id) {
...
function TheCPGroup_Final_SS_art_label($variables) {
  $element = $variables['element'];
...
function TheCPGroup_Final_SS_form_element_label($variables) {
  $art_label = TheCPGroup_Final_SS_art_label($variables);
...
function TheCPGroup_Final_SS_form_element($variables) {
  $element = &$variables['element'];
  // This is also used in the installer, pre-database setup.
  $t = get_t();
...
function TheCPGroup_Final_SS_pager($variables) {
  $tags = $variables['tags'];
  $element = $variables['element'];
  ...
  if ($pager_total[$element] > 1) {
    ...
    return '<h2 class="element-invisible">' . t('Pages') . '</h2>' . TheCPGroup_Final_SS_art_item_list(array(
      'items' => $items,
...
function TheCPGroup_Final_SS_art_item_list($variables) {
  ...
  if (!empty($items)) {
    ...
    foreach ($items as $i => $item) {
      ...
      if (count($children) > 0) {
        // Render nested list.
        $data .= TheCPGroup_Final_SS_art_item_list(array('items' => $children, 'title' => NULL, 'type' => NULL, 'attributes' => $attributes));
      }
      if ($i == 0) {
